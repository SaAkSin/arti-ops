...
arti-ops/
 └── tests/
      └── test_bookstack.py  # BookStackToolset 연동 및 파싱 로직 테스트
```
...
BookStack(사내 위키) API 리소스를 읽어 오거나 결과물을 업로드(Publish)하는 `BookStackToolset` 의 동작 성공 여부를 검증합니다. 통신 지연 혹은 토큰 만료의 이슈를 배제하기 위해 `pytest.fixture`의 `monkeypatch`를 통해 환경변수(`BOOKSTACK_API_URL` 등)를 가짜(Mocked) URL로 오버라이드하여 동작시킵니다.

*   **`test_fetch_policies_global`**: 전사 공통 가이드(L1) 조회 로직 검증.
*   **`test_fetch_policies_workspace_success`**: 워크스페이스 전용 룰(L2)의 성공적 조회 검증 (Project ID 주입).
*   **`test_fetch_policies_workspace_missing_id`**: 워크스페이스 조회 시 필수 파라미터가 비어있을 때 발생하는 예외 에러 반환 검증.
*   **`test_publish_sync_report`**: 배포 이후 구성된 Diff 데이터를 '릴리즈 노트'로 Write-Back 퍼블리싱하는 과정 검증.
