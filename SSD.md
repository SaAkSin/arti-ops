...
[ ADK Core Layer (Python) ]
  ...
  ├── 🧠 SkillArchitect : 수집된 컨텍스트와 사용자의 지시를 융합, 기존 위키 내용을 바탕으로 현재 프로젝트에 맞게 변형/병합(Merge)하여 Rule/Skill 생성 기획
...
2. 하단 프롬프트에 지시 입력 (예: `s 1` 또는 `s 시작` 입력 시 로컬-위키 융합 매크로 자동 실행, `"r"`: 세션 초기화, `"u"`: 로컬 에셋 위키 배포 다이얼로그 노출).
...
4. **Architect**가 지시와 컨텍스트를 바탕으로 `.agents/skills/...` 및 `.agents/rules/...` 규격에 맞춰 내용 기획. (위키 정책이 베이스일 경우 현재 로컬 프로젝트 성격에 맞춰 융합/변형 생성). 실시간 스트림 파싱을 통해 타겟 파일의 경로를 렌더링한다.
