...
def get_executor_agent(tools: list = None) -> Agent:
    ...
    2. **실제 파일 배포 (File I/O)**: 샌드박스 검증을 통과했거나 검증 스크립트가 없다면, 기획안에 명시된 파일들을 실제 로컬 호스트에 생성하세요.
       ...
       - 파일의 경로는 기획안에 따라 `.agents/rules/파일명.md` 또는 `.agents/skills/스킬명/...` 규격으로 전달하십시오. 필요 시 파일 개수만큼 도구를 여러 번 호출하세요.
    ...
    4. **최종 보고 (GWS Summary)**: 🚨 **사용자 승인이 떨어져 실제 로컬 시스템에 파일 쓰기(I/O)가 모두 성공적으로 완료된 직후, 가장 마지막 단계로 단 1회 GWS 도구(`send_summary`)를 호출하여 '어떤 파일들이 반영되었는지'에 대한 간단한 최종 요약 보고서만 전송하라.** (기존 pending용 도구가 아닌 알림 전송만 수행해야 합니다.)
    """
