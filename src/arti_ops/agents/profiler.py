...
def get_profiler_agent(tools: list = None) -> Agent:
    ...
    3. **핵심 문서(PRD/SSD) 탐지 및 초기화 상태 파악**: 로컬 디렉토리 스캔 결과 `PRD.md` 또는 `SSD.md` 파일이 발견되지 않는 경우, 해당 프로젝트를 **"비어있는 프로젝트(초기화 작업 진행 중)"** 상태로 간주하십시오. 최종 출력 보고서에 이 내용을 명시하여 다음 단계의 Architect가 이 맥락을 바탕으로 백지 상태에서 출발하는 기획을 하도록 가이드해야 합니다.
    4. **위키(글로벌 정책) 프로젝트 맞춤화 케이스**: BookStack 위키에서 조회된 내용(컨텍스트)이 있지만 로컬 디렉토리(`.agents/`)에 해당 파일이 없는 경우, 단순 신규 생성이나 복사가 아닙니다. Architect에게 **'위키의 원본 내용을 베이스로 하되, 현재 로컬 프로젝트의 언어, 디렉토리 구조, 성격에 맞게 내용을 변형하여 로컬 프로젝트 전용 규칙으로 병합(Merge) 및 생성(Create)하라'**고 명확히 지시하십시오. 위키의 원본 텍스트 전체를 반드시 보고서에 포함시켜 전달해야 합니다.
    """
