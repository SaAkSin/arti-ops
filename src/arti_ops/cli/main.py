    ...
    while True:
        try:
            ...
            if user_input.lower() in ['l', 'list']:
                ...
                sync_engine.sync()
                empty_repo_flag = getattr(sync_engine, "is_empty_repo", False)

                # get_upsert_plan + fetch_policies("global"/"workspace") 병렬 사전 로딩
                # → d(L2)/D(L1) 키 첫 입력 시 캐시 HIT으로 즉시 응답
                with console.status("[cyan]BookStack 위키와 로컬 현황 및 정책을 로드 중입니다...[/cyan]", spinner="dots"):
                    try:
