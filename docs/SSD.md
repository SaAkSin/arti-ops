...
[ TUI Layer (Local Console) ]
  └── 🖥️ Native IME 기반 Full-screen App
      ...
      ├── ⌨️ Input Prompt (하단) : 상시 활성화된 사용자 대화 및 유틸 명령어 (r: 초기화, u: 위키 통합 배포/가져오기, q: 종료 명령)
      └── ⚡ Event Handler : 'q', 'Ctrl+C' 즉시 종료 글로벌 바인딩

[ ADK Core Layer (Python) ]
  ├── ⚙️ Pipeline Engine : 이벤트 기반 비동기 흐름 제어 및 로컬 승인(HITL) 루프 관리
  ├── 🕵️ ContextProfiler : BookStack 정책(Read) + 현재 실행 경로(로컬 파일, 언어, 구조 등) 현황을 병합 스캔하여 위키-로컬 간 엣지 케이스 식별
  ├── 🧠 SkillArchitect : 수집된 컨텍스트와 사용자의 지시를 융합, 기존 위키 내용을 바탕으로 현재 프로젝트에 맞게 변형/병합(Merge)하여 Rule/Skill/Workflow 생성 기획
...
[ Integration Layer (Tools) ]
  ├── 📚 BookStackToolset : 정책/워크플로우 Fetch 및 양방향 동기화 처리
...
2. 하단 프롬프트에 지시 입력 (예: `s 1` 또는 `s 시작` 입력 시 융합 작업 수행, `"r"`: 세션 초기화, `"u"`: 로컬 에셋 위키 배포 및 Workflow 가져오기 모드 노출).
3. **Profiler**가 로컬 파일 컨텍스트와 BookStack 정책/워크플로우를 수집. 단, 직전 대화 혹은 저장된 `sessions.db`의 컨텍스트로 충분하다고 판단한 경우 캐시 히트 메시지(`💡 이전 세션(sessions.db) 기억을 불러왔습니다.`)를 노출.
4. **Architect**가 지시와 컨텍스트를 바탕으로 `.agents/skills/...` 및 `.agents/rules/...`, 추가로 `.agents/workflows/...` 규격에 맞춰 내용 기획 및 실시간 스트림 파싱.
5. **Verifier**가 무결성 검증 후, TUI 화면에 **[반영 예정 파일 목록 및 상세 내용 요약 보고서]**를 제출하고 파이프라인 대기 상태 전환.
6. 사용자가 화면의 보고서를 검토 후 하단 프롬프트에 `승인(y)` 입력 시 **Executor** 가동.
...
database "📚 BookStack API\n(SSOT: Policy & Workflows)" as Wiki #FDEBD0
...
activate CLI
CLI -> Profiler : 환경 및 정책/Workflows 수집 지시
activate Profiler

alt LLM 판단: "기존 세션만으로 충분함" (Cache Hit)
    Profiler -> CLI : 💡 이전 세션(sessions.db) 기억 로드
else 정밀 스캔 필요
    Profiler -> Wiki : [RestApiTool] 정책/Workflow 마크다운 반환
    Profiler -> CLI : [FileIOTool] 로컬 디렉토리 독해 (L3)
end

Profiler -> Architect : 수집/로드된 컨텍스트 및 지시사항 전달
deactivate Profiler

== 2. 에이전트 기획 및 자율 검증 (Planning & Verification) ==
activate Architect
Architect -> Architect : 전역 룰/Workflows를 준수하며\n프로젝트 특화 요소를 융합 생성
...
alt 'y' 승인
    ...
    activate Executor
    Executor -> CLI : [FileIOTool] 로컬 .agents/ 폴더에\nI/O 배포 수행 (workflows 포함)
    CLI --> Executor : 저장 완료
    Executor -> User : [GwsSummaryTool] 배포 결과 GWS 전송
