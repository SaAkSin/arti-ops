...
- **🚫 제약사항 1 (패키지)**: 파이썬 패키지를 추가할 때 글로벌 `pip`를 사용하지 말고, 항상 `uv add <package>` 명령을 사용해 `pyproject.toml`에 명시적으로 반영되도록 하십시오. OS 레벨과 완벽히 격리된 환경을 보장해야 합니다.
- **🚫 제약사항 2 (클린 환경)**: 테스트, 디버깅, 추론(Reasoning) 목적으로 임시 생성한 파일(예: `test_*.py`, `output.txt` 등)을 작업 종료 후 루트 디렉토리에 절대 방치하지 마십시오. 목적을 달성한 즉시 삭제해야 합니다.
